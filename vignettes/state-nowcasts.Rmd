...
```{r httptest-setup, include = FALSE, eval = TRUE}
old_epidatr_use_cache <- Sys.getenv("EPIDATR_USE_CACHE", unset = NA_character_)
Sys.setenv(EPIDATR_USE_CACHE = "FALSE")
library(httptest2)
...
```{r httptest-teardown, include = FALSE, eval = TRUE}
end_vignette()
if (is.na(old_epidatr_use_cache)) {
  Sys.unsetenv("EPIDATR_USE_CACHE")
} else {
  Sys.setenv(EPIDATR_USE_CACHE = old_epidatr_use_cache)
}
```
